...
**One pattern library (JSON file) per enum.**

| Enum | Pattern library |
|---|---|
| `Unit` | `unit_phrases.json` |
| `StatementType` | `statement_heading_patterns.json` |
| `FilingType` | `filing_type_patterns.json` |
| `Section` | `section_heading_patterns.json` |
| `NumericNotation` | `numeric_notation_patterns.json` |
| `Period` (not an enum but same pattern) | `period_phrase_patterns.json` |

Canonical copies live at `Brain/Knowledge/Model Schema/pattern_libraries/`. Each skill that consumes them keeps a copy in its own `references/` folder (e.g. `~/.claude/skills/financials-extract/references/`).
...
- absorbs micro-variance without any pattern-library edit
...
| Score | Action |
|---|---|
| ≥ 95 | Silent auto-append to the pattern library |
...
- On confirmed novel match, write the new variant back to the JSON file
...
```json
{
  "entries": {
    "quarterly": {
      "canonical": "QUARTERLY",
      "regex_patterns": [
        { "pattern": "q(\\d)\\s*[\\'']?(\\d{2,4})", "captures": { "1": "fiscal_quarter", "2": "fiscal_year" } },
        { "pattern": "(\\d)q\\s*[\\'']?(\\d{2,4})", "captures": { "1": "fiscal_quarter", "2": "fiscal_year" } },
        { "pattern": "quarter\\s+(\\d)\\s*,?\\s+(\\d{4})", "captures": { "1": "fiscal_quarter", "2": "fiscal_year" } },
        { "pattern": "(first|second|third|fourth)\\s+quarter\\s+(\\d{4})", "captures": { "1": "fiscal_quarter_word", "2": "fiscal_year" } }
      ]
    }
  }
}
...
Sits at the top level of each pattern library file alongside `entries`.

---

## Worked example — new variant never seen before

Filing says: `"Statement of Consolidated Income"` — not in the library yet.
...
3. **Auto-append** the new phrase to the `variants` list in `statement_heading_patterns.json`
4. **Next run** with the same phrase is an exact match — zero compute

---

## Why the JSON itself is validated by Pydantic

Pattern files are loaded through `PatternLibrary.model_validate_json()`. This:
1. Rejects malformed JSON at load time, not mid-extraction
2. Enforces `no_duplicate_variants` — the same phrase can't appear in two entries
3. Enforces `fuzzy_threshold` is in range 0-100
4. Provides clear error messages if someone hand-edits the file badly
...
| **Location** | `pattern_libraries/*.json` (+ skill `references/` copies) | `tickers/{ticker}/decisions_ledger.json` |
...
3. **New company later, different phrasing**: keyword match catches most of it without any pattern-library edit
