...
- ~~**CF orphan rows**~~ **FIXED.** Older-only items now slot into their section based on the anchor item that preceded them in the filing where they last appeared. `resolve_row_positions` maintains a per-filing advancing anchor (`last_placed_idx`) that never regresses; when an already-placed rule_id is encountered at an earlier combined-order position (happens when a rule_id has aliases that appear twice in a single filing, e.g. `Income taxes payable` + `Other current liabilities` both alias `GEN-CF-017`), the anchor stays at the most-recent insertion point so subsequent new items don't push already-inserted ones to the end.
- ~~**Extractor section mis-tagging**~~ **NOT A BUG — stale raw JSON.** Diagnosis: the `raw_2024_10K.json` in `Model Output/` was generated by an earlier version of extract.py that pre-dated the subtotal-flip feature. The current extract correctly flips CA→NCA on "Total current assets" and CL→NCL on "Total current liabilities", and handles the section-header rows for CURRENT_LIABILITIES, MEZZANINE, EQUITY via the section-pattern library. Re-extracted both filings fresh at end of session; section tagging is correct across the board. The defensive workarounds added earlier (reconcile's unique-candidate skip, model-write's filing_section bucketing hint) stay — they cost nothing and are useful belt-and-suspenders for edge cases with other tickers.
