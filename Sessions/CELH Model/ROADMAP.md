...
18e. **Calculation-linkbase-driven subtotal detection.** Replace `IXBRL_SUBTOTAL_CONCEPTS` allowlist + walker subtotal-transition text patterns with structural reads of the iXBRL calculation linkbase (subtotals = parents of summands in calc tree). Biggest change in the sprint — touches walker logic. Saved for last to amortize verification through the snapshot harness.
