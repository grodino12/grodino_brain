...
15. **Formalize the ticker onboarding doc** at `Brain\Knowledge\Model Schema\05_ticker_onboarding.md`.
16. **iXBRL label linkbase parsing** (long-term) — recover filer's actual visual labels per concept; eliminates need for ticker-specific overrides.
17. **`financials-validate/SKILL.md` description stale** — still says "BS-1..BS-7, CF-1/CF-2, X-1..X-4". Update to the 10-rule reality.
18. **Refresh playgrounds + LS_KEY v10 → v11** for: variant collapse, paren-of-value sign, CF-2/3/4, row consolidation, first-filing-wins, layered model-write checks, .cache/ convention. Carried per active propagating rule.
19. **Cross-filing CF tie-out micro-warnings on PG** ($2-21M gaps on 4 periods). Acceptable for now; revisit if larger gaps appear elsewhere.
20. **CELH regression** — pending against the new architecture.
