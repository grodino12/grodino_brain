...
| **Generic cross-ticker library** | **115 entries** (109 + 6 novel-triage adds 2026-04-25 third session: GEN-BS-042 Par Value, GEN-BS-043 Treasury Shares memo, GEN-CF-050 Extinguishment, GEN-CF-051 Other WC, GEN-IS-024 Impairment IS, alias adds on GEN-IS-009 / GEN-CF-003) | Grow per novel triage protocol; restore OCI when 4th statement ships |
| **Sign-convention architecture** | **Replaced with paren-of-value visual sign 2026-04-25 (third session).** `_is_parens_negative()` walks the iXBRL fact element's surrounding text in the rendered HTML — single source of truth. Drops sign-attr / negatedLabel inference (filer-by-filer inconsistent). Library `sign_convention` retained for IS-only keyword detection on split-direction lines (Income Tax / FX / Other Non-Op) | Stable |
| **Validator architecture** | **10 filer-tie rules 2026-04-25 (third session)** — BS-1..BS-5 (TCA/TA/TCL/TL/TSE), IS-4 (NI), CF-1 (Net Change in Cash), **CF-2/CF-3/CF-4 (CFO/CFI/CFF section subtotals — new)**. Plus slice-alignment fix in `group_items_by_statement` (was iterating ALL raw statements but slicing kept-only items, drifting indices on every 10-Q). Plus utf-8 stdout reconfigure on Windows (cp1252 was crashing on Δ / em-dash mid-loop and skipping the JSON write) | Stable |
...
**Revised critical path:** ~~`model-write`~~ ✓ → ~~generic-library migration~~ ✓ → ~~`model-calc` drivers + forecast~~ ✓ → ~~fix forecast BS balance gap~~ ✓ → ~~SEC EDGAR ingestion + iXBRL extractor~~ ✓ → ~~quarterly pipeline~~ ✓ → ~~triage PG 107-novel count~~ ✓ → ~~Option B architecture~~ ✓ → ~~sign-convention end-to-end~~ ✓ → ~~library splits~~ ✓ → ~~QTR pivot to YTD durations~~ ✓ → ~~model-write QTR parity port~~ ✓ → ~~merge two extract skills into one~~ ✓ → ~~sign-convention reduced to abs-based 3-value with IS keyword detection~~ ✓ → ~~library splits recombined~~ ✓ → ~~validators: state machines + English prose anchors → section-driven + canonical_label~~ ✓ → ~~UNCLASSIFIED enforcement at reconcile~~ ✓ → ~~LibraryEntry Pydantic guard~~ ✓ → ~~framework audit~~ ✓ → ~~refresh playgrounds~~ ✓ → ~~PG end-to-end on new architecture~~ ✓ → ~~validator trim 18 → 7 filer-tie rules~~ ✓ → ~~PG 3-year cross-year backfill (3 10-Ks + 11 10-Qs)~~ ✓ → ~~variant collapse (one library entry serves both 10-K + 10-Q)~~ ✓ → ~~paren-of-value as authoritative sign (drops sign-attr / negatedLabel inference)~~ ✓ → ~~CF-2/CF-3/CF-4 section validators + slice-alignment fix + utf-8 stdout~~ ✓ → ~~first-filing-wins dedup + row consolidation by canonical_label + layered model-write checks~~ ✓ → **multi-ticker validation (KO/PEP/COST etc.) to confirm framework generalizes** → **break OCI into 4th statement** → **extend model-calc to quarterly drivers** → **extract generic forecast-rules JSON** → **formalize ticker onboarding flow** → (later) cross-model integration.
...
12. **NEXT SESSION OPENS WITH: HTM-only iXBRL extractor rewrite.** Attempted dimensioned-only-concept synthesis 2026-04-26 (second session). The narrow Intangibles case worked ($1.39B recovered; BS-2 closes mathematically), but the precondition (R-file defref anchor presence) systematically routed footnote/disclosure facts onto primary statements (related-party AR appearing as a phantom BS row, per-acquisition consideration breakouts as phantom CF rows). User flagged this as the 5th time R-file dependence has caused phantom data. Reverted the synthesis. **Decision: drop R-file dependence entirely from the primary-statements pipeline.** Walk the primary HTM directly — table-by-table, row-by-row — pulling visual labels from row first cells, displayed values from cell text content (parens-negative authoritative), concept names from `<ix:nonFraction name="...">` tags inside cells. Drops `build_concept_statement_map` / `extract_anchors_from_report` / `extract_sections_from_report` / R-file fetch pipeline. The dimensioned-only synthesis problem dissolves because cell text IS the value. Per `feedback_no_rfiles_for_financials.md` — R-files acceptable for future analytical surfaces (charts, footnote browsers, disclosure dashboards), never for primary statement extraction. Detail in latest handoff §1-4.
13. **Break OCI into its own worksheet (4th statement).** Carried. CI statements skipped today. Scope: add `COMPREHENSIVE_INCOME` to StatementType enum, route CI in iXBRL extractor, OCI/QTR OCI sheets in model-write, OCI library entries restored.
14. **Extend `model-calc` to quarterly drivers.** Currently annual-only.
15. **Extract `pattern_libraries/generic_forecast_rules.json`** from `calc.py`. Blocked on §14.
16. **Formalize the ticker onboarding doc** at `Brain\Knowledge\Model Schema\05_ticker_onboarding.md`.
17. **iXBRL label linkbase parsing** (long-term) — superseded by §12 (HTM-only walker pulls visual labels straight from row first cells).
18. **`financials-validate/SKILL.md` description stale** — still says "BS-1..BS-7, CF-1/CF-2, X-1..X-4". Update to the 10-rule reality.
19. **Refresh playgrounds + LS_KEY v9 → v10** for: HTM-only extractor (post-§12), variant collapse, paren-of-value sign, CF-2/3/4, row consolidation, first-filing-wins, layered model-write checks, .cache/ convention. Carried per active propagating rule.
20. **Cross-filing CF tie-out micro-warnings on PG** ($2-21M gaps on 4 periods). Acceptable for now; revisit if larger gaps appear elsewhere.
21. **CELH regression** — pending against §12 + the new architecture.
