...
18e. **Calculation-linkbase loader (partial — infrastructure shipped, allowlist replacement deferred).** Built `_load_calc_subtotal_concepts()` in `ixbrl_path.py` — fetches `*_cal.xml` from cache (one-time bulk-fetched 26 cal.xml files for CELH+PG), parses calculationArc elements, returns set of parent concepts. Pipeline-attempt revert (2026-04-27): tried using calc-tree parents as the routing-time subtotal signal. **Premise too broad** — calc-tree parents include mid-statement aggregations like `IncomeLossFromContinuingOperationsBeforeIncomeTaxesExtraordinaryItemsNoncontrollingInterest` (Pre-Tax Income), which the pipeline currently treats as line items + applies SUM formulas at model-write time. Tagging those subtotal at extract forces strict-mode alias lookup, which fails on filer label variations ("Net income before income taxes" → no exact alias). Reverted walker to use `IXBRL_SUBTOTAL_CONCEPTS` allowlist; loader stays in place for future use cases (subtotal **synthesis** when filer reports only summands without a parent — calc linkbase declares exactly which children sum to which parent and with what weight). Proper structural replacement for the allowlist would use the **presentation linkbase**'s `preferredLabel="totalLabel"` attribute (only flags rows the filer chose to render as totals); that's the next-step design.
...
20. **`_pre.xml` linkbase removal cleanup pass** — current code keeps it dormant; do a delete-pass once PG also passes. Per `feedback_no_rfiles_for_financials.md`.
