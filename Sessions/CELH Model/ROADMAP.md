...
last_session: "April 26th PG Onboarding + Workbook Tie-Out Validator Session"
next_objective: "Tier C heuristic hardening sprint — replace ticker-tuned regex/keyword/allowlist patterns with structural signals from XBRL, gated by a snapshot regression harness, BEFORE onboarding any new tickers"
---
...
|---|---|---|
| **Tier C heuristic hardening sprint** (NEW 2026-04-27) | **Planning complete; implementation not started.** Five-stage sprint to replace ticker-tuned heuristics (regex/keyword/allowlist) with structural signals (XBRL concept→canonical, calc-linkbase parent-of-summands, NCI presence at filing-read time). Gated by snapshot harness locking CELH (12) + PG (14) outputs. **No new tickers added until sprint complete** to prevent compounding drift. See Active §18a-e. | Build snapshot harness (§18a) — ~2-3hr orchestration script + golden generation; then proceed to §18b-e |
...
**Revised critical path:** ~~`model-write`~~ ✓ → ~~generic-library migration~~ ✓ → ~~`model-calc` drivers + forecast~~ ✓ → ~~SEC EDGAR ingestion + iXBRL extractor~~ ✓ → ~~quarterly pipeline~~ ✓ → ~~PG 14-filing clean baseline~~ ✓ → ~~paren-of-value as authoritative sign~~ ✓ → ~~CF-2/CF-3/CF-4 section validators~~ ✓ → ~~first-filing-wins dedup + layered model-write checks~~ ✓ → ~~CELH multi-ticker onboarding (3 10-Ks + 9 10-Qs through new architecture)~~ ✓ → ~~HTM-only iXBRL walker rewrite~~ ✓ 2026-04-26 → ~~CELH 12-filing clean~~ ✓ 2026-04-26 → ~~multi-cell paren detection~~ ✓ 2026-04-26 → ~~IS-1/IS-2/IS-3/IS-5 cascade validators~~ ✓ 2026-04-26 → ~~Convertible Preferred → Equity layout fold + Net Change in Cash always-synthesized~~ ✓ 2026-04-26 → ~~PG 14-filing clean on new HTM-walker (0 novels, 0 fails, 1,178-cell workbook)~~ ✓ 2026-04-26 (PG session) → ~~Label-only library matching (concept-name fallback dropped)~~ ✓ 2026-04-26 (PG session) → ~~Section-mismatch filter applied uniformly in select_entry (caught DTL→assets routing)~~ ✓ 2026-04-26 (PG session) → ~~Workbook tie-out validator in model-write~~ ✓ 2026-04-26 (PG session) → ~~Per-period tax-sign correction in extract.py~~ ✓ 2026-04-26 (PG session) → ~~CELH single-NI fix (move bare-NI aliases to GEN-IS-022)~~ ✓ 2026-04-26 (PG session) → **Tier C heuristic hardening sprint** (snapshot harness for CELH+PG → concept-derived canonical label fallback → NCI structural detection → drop "charge" keyword scan → calc-linkbase-driven subtotal detection) → **third ticker onboarding (PEP / KO)** → **break OCI into 4th statement** → **extend model-calc to quarterly drivers** → **extract generic forecast-rules JSON** → **formalize ticker onboarding flow** → (later) cross-model integration.
...
18. **NEXT SESSION OPENS WITH: Tier C heuristic hardening sprint.** Replace ticker-tuned heuristics with structural signals BEFORE adding any new tickers. Rationale: heuristics added during CELH+PG hardening (regex on label clutter, keyword scan for "charge", allowlist of subtotal concepts, library-author-time NI alias routing) carry drift risk — a future ticker could silently break CELH or PG. Structural signals (XBRL concept→canonical, calc-linkbase parent-of-summands, NCI presence at filing-read time) eliminate the drift mode. Sprint is gated by a snapshot harness that locks current CELH+PG outputs so any regression is mechanically detected.
18a. **Snapshot harness for CELH+PG regression lock.** Stand up `Brain\Knowledge\Model Schema\_regression\run.py` + frozen goldens at `_regression\goldens\{TICKER}\`. Reruns extract→reconcile→validate→model-write per filing into temp dir, diffs against goldens. Numeric tolerance $1. Workbook diffed as cell-map snapshot (`{sheet, row_label, col_label} → value`), not xlsx binary (nondeterministic metadata). `--accept {TICKER}` flag for explicit golden-overwrite. ~2-3 hours of work; gates everything below.
18b. **Concept-derived canonical label fallback.** In `ixbrl_path.py` (or `lookup.py`): when raw_filing_label matches no alias but the iXBRL concept maps to a canonical, use the canonical's label. **Eliminates `_DATE_ONLY_LABEL_RE` outright.** Demotes `CLUTTER_RE` + `TRAILING_PAREN_RE` to non-load-bearing (concept path wins first).
18c. **NCI structural detection for bare-NI routing.** Detect at filing-read time whether the filing has `NoncontrollingInterest`-bearing concepts. Route bare-NI label aliases to GEN-IS-022 for non-NCI filers, GEN-IS-011 only for NCI filers based on attribution. **Removes the library-author-time decision currently encoded by alias placement.**
18d. **Drop "charge" keyword scan; sign comes from canonical only.** Audit every "charge"-labeled item across CELH+PG; verify each routes to a canonical with `sign_convention=negative`. Then delete `"charge"` from `_NEGATIVE_KEYWORDS` in `extract.py`. Sign becomes a property of the canonical, not a label-text scan.
18e. ~~**Presentation-linkbase-driven subtotal detection.**~~ Shipped 2026-04-27. Walker reads each filing's `*_pre.xml` via `_load_presentation_total_concepts()` — collects every concept tagged `preferredLabel=totalLabel` (the filer's own rendering intent). Set is unioned with `IXBRL_SUBTOTAL_CONCEPTS` (kept as floor) and used in walker subtotal detection. **IS strict-mode dropped** per `key.stmt_type != INCOME_STATEMENT` guard — IS canonicals (Pre-Tax, Operating, Net Income) are semantically distinct enough that fuzzy alias match works; the BS strict-mode collision risk ("Total current liabilities" 88%-fuzz "Other current liabilities") doesn't apply on the IS. CELH+PG harness: zero diffs (existing canonical row_type=subtotal declarations on GEN-IS-003/005/009/022 already drove the same outcome via canonical override; pre.xml just adds drift resistance for future filers using subtotal concepts not in our allowlist). Calc-linkbase loader (`_load_calc_subtotal_concepts`) stays in place for the future synthesis use case (when a filer reports only summands without a parent — calc linkbase declares parent-of-summands relationships and weights). Earlier attempt to use calc parents as the subtotal signal was reverted: premise too broad, calc parents include mid-statement aggregations the pipeline currently treats as line items.
19. **Onboard third ticker (PEP or KO) AFTER sprint completion.** Validate cross-ticker generality of the now-bomb-proof framework. Workflow: mkdir ticker root, drop config.json, run extract→reconcile→validate→model-write, triage novels per `feedback_novel_triage_protocol.md`. Snapshot harness extends to include the new ticker once it's clean.
...
21. **Break OCI into its own worksheet (4th statement).** Carried. Walker currently STOPS on OCI header; emit them as `StatementType.COMPREHENSIVE_INCOME` once that StatementType ships.
22. **Extend `model-calc` to quarterly drivers.** Currently annual-only.
23. **Extract `pattern_libraries/generic_forecast_rules.json`** from `calc.py`. Blocked on §22.
24. **Formalize the ticker onboarding doc** at `Brain\Knowledge\Model Schema\05_ticker_onboarding.md`.
25. **`financials-validate/SKILL.md` description stale** — still says "10 filer-tie rules". Update to "14 rules: BS-1..5, IS-1..3 + IS-5, IS-4, CF-1..4". Add the IS cascade rationale (catches sign-flipped opex rows).
26. **Refresh playgrounds + LS_KEY v9 → v10** for: HTM-only walker (R-file nodes deleted), multi-cell paren detection, IS-1..3+IS-5, mezz→equity fold, NetChange always-synth, .cache/ convention, label-only matching, workbook tie-out validator, per-period tax-sign correction, **plus sprint deliverables (snapshot harness node, structural-signal preference, calc-linkbase reads)**. Carried per active propagating rule.
...
Surface area to watch when running PEP (deferred until §18 sprint complete):
